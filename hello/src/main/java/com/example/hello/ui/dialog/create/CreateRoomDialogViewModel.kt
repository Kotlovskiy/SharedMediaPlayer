...
@HiltViewModel
class CreateRoomDialogViewModel @Inject constructor(
    ...
    fun clickConfirm() {
        viewModelScope.launch {
            when(val result = inviteRepository.createRoom(_state.value)) {
    ...
    private suspend fun sendErrorEffect(error: NetworkError) {
        when(error) {
            NetworkError.Conflict -> _effect.send(Effect.ShowConflictError)
            NetworkError.Forbidden -> _effect.send(Effect.ShowUnknownError)
            ...
            NetworkError.NotFound -> _effect.send(Effect.ShowUnknownError)
